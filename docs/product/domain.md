...
| **Backend / adapter** | The agent-vendor seam: `backends/<name>.sh` (`run` / `resume` / `capabilities`). Production adapters include Claude Code direct and its fixed-family DeepSeek, Kimi, and GLM wrappers, plus the `codex-cli` adapter (OpenAI Codex CLI); `mock.sh` is a hermetic seam proof. |
...
- **Plan-affecting config comes from the default branch.** `runner`,
  `trusted_actors`, `required_capabilities`, `image`, `[reviewer]`, and `[verify]`
  are never read from a dispatched branch.
