...
class FakeGh:
    ...
    def when(self, needles: list[str], rc: int = 0, out: object = ""):
        self.rules.append(
            (needles, rc, out if isinstance(out, str) else json.dumps(out))
        )
        return self
    ...
    def __call__(self, args: list[str]) -> tuple[int, str, str]:
        ...
        joined = " ".join(args)
        for needles, rc, out in self.rules:
            if all(needle in joined for needle in needles):
                return rc, out, "" if rc == 0 else "denied"
        return 1, "", "denied"
