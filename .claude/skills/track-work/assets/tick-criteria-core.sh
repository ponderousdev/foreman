#!/usr/bin/env bash
# tick-criteria-core.sh — shared implementation for the scoped acceptance
# criterion tickers. This file is deliberately not an allowlisted entry point.
# anything else to it.
#
...
# closed ticks must be CLOSED/COMPLETED and do not depend on a released claim.
# Their wrappers select those modes structurally before this implementation runs.
#
...
usage() {
    echo "Usage: $0 --mode open|closed --repo owner/repo --issue N [--match TEXT]... [--index K]... [--dry-run]" >&2
    exit 2
}
...
assert_authorized() {
    ...
    *)
    ...
    OPEN | open)
        [ "$mode" = open ] || {
            echo "tick-criteria: this command is for completed closed issues only" >&2
            exit 1
        }
        ;;
    CLOSED | closed)
        [ "$mode" = closed ] || {
            echo "tick-criteria: $repo#$issue is closed — use tick-completed-criteria.sh for a completed post-merge tick" >&2
            exit 1
        }
