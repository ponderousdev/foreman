...
`ponderousdev-ci` GitHub App with short-lived tokens, not this PAT. Don't
...
`ponderousdev/…` repos, and vice versa.
...
For repos in `ponderousdev`, `task setup:github` does this and is idempotent:

```bash
task setup:github   # among other settings, adds the bot as a Write collaborator
```

Prefer it over clicking — it is the documented path and safe to re-run. To grant
**read-only** instead (for a repo the bot only needs to clone, e.g. a private
dependency), deviate deliberately:

```bash
gh api repos/ponderousdev/<repo>/collaborators/evanharmon1-bot \
  --method PUT -f permission=pull
```

Verify what the bot actually has:

```bash
gh api repos/ponderousdev/<repo>/collaborators/evanharmon1-bot/permission \
  --jq '.permission'
```
...
git ls-remote https://github.com/ponderousdev/<repo> HEAD   # auth works for git, not just gh
```
