...
| **Backend / adapter** | The agent-vendor seam: `backends/<name>.sh` (`run` / `resume` / `capabilities`). Production adapters include Claude Code direct and its fixed-family DeepSeek wrapper; `mock.sh` is a hermetic seam proof. |
